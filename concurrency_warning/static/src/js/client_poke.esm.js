    ...
    start (env, {action, notification, user}) {
        const _doNotify = debounce(function (payload) {
            notification.add(
                _.str.sprintf(
                    env._t(
                        "This record has been changed by another user (%s) " +
                        "since you opened it. The record has been " +
        ...
        function _handleNotifications (notifications) {
            for (const {payload, type} of notifications) {
                ...
                if (
                    action.currentController.props.resModel ===
                    payload.model && payload.ids.includes(
                        action.currentController.props.resId) &&
